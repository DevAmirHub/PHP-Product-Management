 ...
 * Displays a single product row in the products table
 * 
 * @var array $product Product data array
 * @var int $offset Pagination offset
 * @var int $index Current index in loop
 */

// Debug: Uncomment to see product data
// print_r($product);exit;

// Product status labels in Persian
$statuses = [
    'draft'     => 'پیش نویس',
    'presale'   => 'پیش فروش',
    'expire'   => 'توقف فروش',
...
$status_label = $statuses[$product_statuses];
?>
<tr>
    ...
    <td>
        <?php if( get_discount_price( $product['price'] , $product['sale_price'] ) ): ?>
        <del>
            <?php echo number_format( $product['price'] / 10) ?>
        </del>
        <?php endif; ?>
        <ins>
            <?php echo number_format( $product['sale_price'] / 10) ?>
        </ins>
    </td>
    <td>
        <?php echo get_discount_price( $product['price'] , $product['sale_price'] ); ?>%
    </td>
    <td>
    ...
    <td>
        <div class="table-flex-col">
            <a href="product-edit.php?id=<?php echo intval( $product['ID'] ) ?>" class="btn-icon edit-product">
            ...
            <a href="?action=delete&product_id=<?php echo $product['ID']; ?>" class="btn-icon delete-product" onclick="return confirm('آیا مطمئن هستید ؟')">
